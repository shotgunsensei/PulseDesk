...
export async function registerRoutes(
  ...
  app.get("/api/invoices/export/quickbooks", requireAuth, requireOrg, async (req: Request, res: Response) => {
    try {
      ...
      res.setHeader("Content-Disposition", `attachment; filename="invoices-quickbooks-${new Date().toISOString().slice(0, 10)}.csv"`);
      res.send(csv);
    } catch (err: any) {
      res.status(500).send(err.message);
    }
